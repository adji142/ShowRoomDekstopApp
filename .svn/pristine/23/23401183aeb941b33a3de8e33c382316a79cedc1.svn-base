...
{
    public partial class PlafonBankUpdate : ISA.Controls.BaseForm 
    {
        public Guid _RowID, _RekeningRowID;
        public String _matauangID;
        public string _Modus;

        public PlafonBankUpdate()
        {
            InitializeComponent();
        }
        public PlafonBankUpdate(Form Caller, Guid RekeningRowID,String MataUangID)
        {
            InitializeComponent();
            _Modus = "Tambah";
        ...
        public PlafonBankUpdate(Form Caller, Guid RekeningRowID, Guid RowID, String MataUangID)
        {
            InitializeComponent();
            _Modus = "Edit";
            _matauangID = MataUangID;
        ...
        private void cmdSAVE_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (Database db = new Database())
                {
                    if (_Modus == "Tambah")
                    {
                        db.Commands.Add(db.CreateCommand("usp_PlafonBank_INSERT"));
                    ...
                    {
                        db.Commands.Add(db.CreateCommand("usp_PlafonBank_UPDATE"));
        ...
        private void LoadEdit()
        {
            ...
            using (Database db = new Database())
            {
                db.Commands.Add(db.CreateCommand("usp_PlafonBank_SEEK"));
                db.Commands[0].Parameters.Add(new Parameter("@RowID", SqlDbType.UniqueIdentifier, _RowID ));
                dt = db.Commands[0].ExecuteDataTable();
            }
            txtTanggal.DateValue = (DateTime)Tools.isNull(dt.Rows[0]["Tmt"], "");
            txtJumlah.Text = Tools.isNull(dt.Rows[0]["plafon_rp"], "").ToString();
        }
        #endregion

        private void PlafonBankUpdate_Load(object sender, EventArgs e)
        {
