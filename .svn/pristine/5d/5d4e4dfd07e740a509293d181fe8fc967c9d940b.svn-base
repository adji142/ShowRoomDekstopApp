...
{
    public partial class frmCF_RealisasiSetoran : ISA.Controls.BaseForm
    {
        List<TextBox> _lTxtJumlah = new List<TextBox>();
        Guid _RowID;

        public frmCF_RealisasiSetoran()
        {
            InitializeComponent();
        ...
        private void RefreshData()
        {
            ...
            using (Database db = new Database(GlobalVar.DBName))
            {
                db.Commands.Add(db.CreateCommand("usp_CF_RealisasiSetoran_LIST"));
        ...
        private void frmCF_RealisasiSetoran_Load(object sender, EventArgs e)
        {
            InitControl();
            SetEnableDisable();
            RefreshDataGrid();
        ...
        private void cmdREFRESH_Click(object sender, EventArgs e)
        {
            ...
            using (Database db = new Database(GlobalVar.DBName))
            {
                db.Commands.Add(db.CreateCommand("usp_CF_RealisasiSetoran_UPDATE"));
                db.Commands[0].Parameters.Add(new Parameter("@PerusahaanRowID", SqlDbType.UniqueIdentifier, GlobalVar.PerusahaanRowID));
                db.Commands[0].Parameters.Add(new Parameter("@Periode", SqlDbType.VarChar, Periode));
