...
{
    public partial class frmLookUpStokMotorTLA : ISA.Controls.BaseForm
    {
        DataTable _dtCari = new DataTable();
        public Class.clsStokMotor Motor { get; set; }
        bool pusat = false;

        #region Konstruktor
        public frmLookUpStokMotorTLA()
        {
            InitializeComponent();
        }

        public frmLookUpStokMotorTLA(DataTable dt)
        {
        ...
        void DBGetList()
        {
            try
            {
                _dtCari = Class.clsStokMotor.DBSearchStokBARU(txtSearch.Text.Trim());
                RefreshGrid();
            }
            catch (Exception ex)
            {
