...
{
    public class clsCostumer:clsBase
    {
        #region FIELDS
		string _namaCostumer;		
        #endregion

        #region PROPERTIES
        public string NamaCustomer { get { return _namaCostumer; } set { _namaCostumer = value; } }
        #endregion
        ...
        public clsCostumer(Guid t_rowID)
        {
            try
            {
        ...
        public clsCostumer(Guid t_rowID, string lookup)
        {
            try
            {
                DataTable dt = DBGetByRowIDALL(t_rowID);
                if ((dt != null) && (dt.Rows.Count > 0))
                {
                    SetFromDataRow(dt.Rows[0]);
                    _clsState = enumClsState.Update;
        ...
        public void SetFromDataRow(DataRow dr)
        {
            ...
            {
                RowID = (Guid)dr["RowID"];
                _namaCostumer = dr["Nama"].ToString();
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            DataTable dt = new DataTable();
                try
                {
                    using (Database db = new Database())
                    {
                        ...
                        Command cmd = db.CreateCommand("usp_Customer_LIST");
                        cmd.Parameters = prm;
        ...
        public static DataTable DBSearchALL(string _searchArg, string _filterArg)
        {
            DataTable dt = new DataTable();
            try
            {
        ...
        public static DataTable DBSearchALL(string _searchArg)
        {
            DataTable dt = new DataTable();
            try
            {
        ...
        public static bool DBFind(Guid t_rowID)
        {
            ...
            try
            {
                DataTable dt = DBTools.DBGetDataTableByRowID("usp_Customer_LIST", t_rowID);
        ...
        {
            return DBTools.DBGetDataTableByRowID("usp_Customer_LIST",t_rowID);
        }

        DataTable DBGetByRowIDALL(Guid t_rowID)
        {
            return DBTools.DBGetDataTableByRowID("usp_Customer_LISTALL", t_rowID);
        }
        #endregion
    }
}
