...
namespace ISA.Showroom.Finance
{
    class SecurityManager
    {
        ...
        {                     
            ...
            {
                var aaa = db.Connection.Database;
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                _TglPassword = DateTime.Parse(Tools.isNull(dt.Rows[0]["TglPassword"], GlobalVar.GetServerDate).ToString());
        ...
        public static bool IsAuthenticate(string userID, string password, string dbName)
        {
            ...
            else
            {
                db = new Database();
                                           ...
                                           // ISAShowrrom Login User
            db.Commands.Add(db.CreateCommand("usp_SecurityUsers_AUTHENTICATE"));
            db.Commands[0].Parameters.Add(new Parameter("@userID", SqlDbType.VarChar, userID));
            db.Commands[0].Parameters.Add(new Parameter("@password", SqlDbType.VarChar, password));
            dt = db.Commands[0].ExecuteDataTable();

                    // ISAFinanceOtoDB untuk dapat settingan aplikasi yg bisa dibuka
            if (dt.Rows.Count > 0)
            {
                db = new Database();
