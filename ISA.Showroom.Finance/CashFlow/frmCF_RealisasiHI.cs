...
{
    public partial class frmCF_RealisasiHI : ISA.Controls.BaseForm
    {
        List<TextBox> _lTxtJumlah = new List<TextBox>();
        Guid _RowID;

        public frmCF_RealisasiHI()
        {
            InitializeComponent();
        ...
        private void frmCF_RealisasiHI_Load(object sender, EventArgs e)
        {
            InitControl();
            SetEnableDisable();
            RefreshDataGrid();
        ...
        private void RefreshData()
        {
            ...
            SetEnableDisable();
            Guid HIRowID = (Guid)Guid.Empty;
            if (dgHI.SelectedCells.Count > 0)
            {
                HIRowID = (Guid)dgHI.SelectedCells[0].OwningRow.Cells["RowID"].Value;
            }
            string Periode = myPeriode.Year.ToString() + myPeriode.Month.ToString().PadLeft(2, '0');
            DataTable dt = new DataTable();
            using (Database db = new Database(GlobalVar.DBName))
            {
                db.Commands.Add(db.CreateCommand("usp_CF_RealisasiHI_LIST"));
                db.Commands[0].Parameters.Add(new Parameter("@PerusahaanRowID", SqlDbType.UniqueIdentifier, HIRowID));
        ...
        private void dgHI_SelectionChanged(object sender, EventArgs e)
        {
            RefreshData();
        }
        ...
        private void cmdREFRESH_Click(object sender, EventArgs e)
        {
            ...
            using (Database db = new Database(GlobalVar.DBName))
            {
                db.Commands.Add(db.CreateCommand("usp_CF_RealisasiHI_UPDATE"));
